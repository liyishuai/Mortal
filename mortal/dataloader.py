...
def batched(iterable, batch_size, *, drop_last=False):
    """Batch a Python iterable of NumPy-compatible records without PyTorch."""
    batch = []
    for entry in iterable:
        batch.append(entry)
        if len(batch) == batch_size:
            yield tuple(np.stack(values, axis=0) for values in zip(*batch))
            batch.clear()
    if batch and not drop_last:
        yield tuple(np.stack(values, axis=0) for values in zip(*batch))


class FileDatasetsIter:
    def __init__(
        self,
        version,
        file_list,
    ...
    def load_files(self, augmented):
        ...
        for start_idx in range(0, len(self.file_list), self.file_batch_size):
            ...
            reserved_size = int(
                (buffer_size - old_buffer_size) * self.reserve_ratio
            )
        ...
        for file in data:
            for game in file:
                ...
                scores_seq = np.concatenate(
                    (grp_feature[:, 3:] * 1e4, [final_scores])
                )
                rank_by_player_seq = (
                    (-scores_seq)
                    .argsort(-1, kind="stable")
                    .argsort(-1, kind="stable")
                )
                ...
                for i in reversed(range(game_size)):
                    if not dones[i]:
                        steps_to_done[i] = (
                            steps_to_done[i + 1] + int(apply_gamma[i])
                        )
